...
## Multiplayer Wi-Fi Scaffolding Addendum

This addendum narrows scope to multiplayer work outside renderer ownership.

Locked scope for this addendum:

- renderer owns interaction, camera, renderers, and some renderer-adjacent data pipelines
- inputs remain engine-oriented
- this addendum focuses on multiplayer scaffolding, modularization, and cleanup for Wi-Fi/LAN readiness
- this addendum avoids risky renderer rewrites, gameplay authority rewrites, or protocol overhauls

### Intent

Prepare the project for the first non-local multiplayer step:

- one host on LAN/Wi-Fi
- one or more clients connecting by host address
- same existing host-authoritative session and lease model
- minimal-risk refactors first

The goal is not to finish full multiplayer here.

The goal is to remove localhost-only assumptions and extract stable engine-facing multiplayer seams so Wi-Fi support can be added without destabilizing current local behavior.

### Locked Direction

1. First remote topology is host-authoritative LAN/Wi-Fi.
2. Localhost remains the default during migration.
3. Renderer-owned systems are out of scope for this addendum.
4. Engine-oriented input work continues separately and should remain compatible with remote clients later.
5. The first shared join UX is an engine-owned join directory screen, not app-specific join panels or implicit auto-join-only behavior.
6. Existing session-token, reconnect-token, and lease patterns should be preserved unless a concrete flaw is found.
7. Current actor-claim behavior may remain program-specific, but the seam around it should become more engine-like.
8. THAUMWORLD keeps character claim/creation after join succeeds; the engine owns connection discovery and selection, not game-specific post-join flow.
9. ASCII painter keeps document bootstrap after join succeeds; the engine owns connection discovery and selection, not painter-specific post-join flow.

### Current Constraints Observed In Code

Current multiplayer foundations already exist:

- session bootstrap and reconnect tokens in `src/shared/multiplayer_session.ts`
- controlled actor claim lease in `src/shared/session_control.ts`
- targeted/public event delivery in `src/event_bridge/main.ts`
- local host probing in `scripts/launcher_common.mjs`

Current blockers to Wi-Fi readiness:

- localhost-only API and bridge assumptions in client and bridge code
- local discovery and remote discovery are not yet owned by one engine join directory
- host/client launcher flow assumes local probing first
- claim/lease naming is still tightly framed around THAUMWORLD actor control
- transport endpoints are not exposed through one shared runtime config
- THAUMWORLD and painter currently use separate join/boot UX paths
- live connection status and persisted connection records are still mixed together in app-specific code

### Non-Goals

- no renderer refactor
- no camera or interaction refactor for this addendum
- no gameplay authority rewrite
- no peer-to-peer networking
- no dedicated-server-first requirement
- no automatic LAN broadcast discovery in the first pass
- no large security/auth redesign
- no protocol rewrite unless needed to create stable seams
- no THAUMWORLD character-select redesign before join succeeds
- no painter save/load/resume redesign under the name of join UX

### Minimal Authority Definition For This Phase

Authority remains intentionally minimal here.

For this phase, the host is authoritative for:

- session issuance and validity
- reconnect/resume validity
- possession or control lease validity
- command acceptance or rejection boundaries

This addendum does not attempt to fully redesign world simulation authority or renderer synchronization.

### Wi-Fi Scaffolding Phases

### Phase 1: Transport Config Extraction

Status: not started

Intent:

Replace hardcoded localhost and `127.0.0.1` assumptions with one shared transport configuration surface while preserving identical current behavior.

Deliverables:

- [ ] define a shared transport config shape for:
- [ ] API base URL
- [ ] event bridge HTTP URL
- [ ] event bridge WS URL
- [ ] slot
- [ ] join target id or host label
- [ ] inventory all localhost-only call sites and route them through transport config
- [ ] keep localhost defaults so current boot flow does not change

Likely files:

- `src/shared/event_bridge_client.ts`
- `src/mono_ui/websocket_client.ts`
- `src/interface_program/frontend_api.ts`
- `src/canvas_app/world_discovery.ts`
- `scripts/launcher_common.mjs`

Success criteria:

- [ ] no multiplayer client path hardcodes `localhost` or `127.0.0.1` directly
- [ ] local behavior still works with default config values

### Phase 2: Engine Connection Core

Status: not started

Intent:

Define one engine-owned connection model that both THAUMWORLD and ASCII painter can consume.

Deliverables:

- [ ] define `EngineConnectionEntry` as the persistent connection record
- [ ] define `EngineConnectionProbeResult` as live, non-persisted probe state
- [ ] derive removability/editability from `kind` instead of storing UI flags on the record
- [ ] synthesize a built-in `local` connection entry rather than persisting it manually
- [ ] move saved manual connection persistence into an engine-owned store
- [ ] keep LAN-discovered connections as future additive ephemeral entries

Suggested engine connection shape:

```ts
type EngineConnectionEntry = {
  id: string;
  name: string;
  host: string;
  kind: 'local' | 'lan_discovered' | 'saved_manual';
  scope: 'local_machine' | 'lan' | 'wifi' | 'internet';
  transport?: {
    api_base_url?: string;
...
type EngineConnectionProbeResult = {
...
- [ ] local connection is always available as an engine-generated entry
- [ ] saved manual connections are shared by both boot flows
- [ ] live status no longer needs to be stored on persistent connection records
- [ ] both programs can consume the same connection types without app-specific wrappers

### Phase 3: Engine Probe + Join Directory

Status: not started

Intent:

Build one engine-owned join directory that combines local and saved manual connections and computes live status at menu refresh time.

Deliverables:

- [ ] centralize `/api/host/status` probing in an engine-owned helper
- [ ] build a join directory that merges:
- [ ] built-in local connection
- [ ] saved manual connections
- [ ] future LAN-discovered connections
- [ ] keep current local host detection as one connection source rather than a separate UX path
- [ ] sort entries for good UX:
- [ ] online local first
- [ ] online LAN next
- [ ] online saved manual next
- [ ] offline saved manual after that

Primary files:

- `src/engine_multiplayer/connection_probe.ts`
- `src/engine_multiplayer/join_directory.ts`
- migration from `src/canvas_app/world_discovery.ts`

Success criteria:

- [ ] the engine can assemble one list for join UI without app-specific discovery glue
- [ ] local auto-detect behavior still exists, but now appears as a list entry in the shared join system
- [ ] probe metadata can carry both THAUMWORLD and painter join hints without changing the persistent connection type

### Phase 4: Engine Session Service Cleanup

Status: not started

Intent:

Treat session bootstrap and reconnect as one reusable engine-grade service surface.

Deliverables:

- [ ] define stable session bootstrap vocabulary around:
- [ ] `client_session_id`
- [ ] `connection_id`
- [ ] `session_token`
- [ ] `reconnect_token`
- [ ] lease timestamps
- [ ] document connect, resume, touch, and invalidate flows
- [ ] keep current storage and semantics intact during cleanup
- [ ] reduce ad hoc session bootstrap logic at call sites

Primary files:

- `src/shared/multiplayer_session.ts`
- interface connect/session endpoints
- client bootstrap callers

Success criteria:

- [ ] one documented session model exists
- [ ] local and remote connection paths can use the same bootstrap flow

### Phase 5: Possession Lease Boundary Cleanup

Status: not started

Intent:

Keep current THAUMWORLD actor-claim behavior, but introduce a cleaner engine-facing boundary for possession/control leases.

Deliverables:

- [ ] document the difference between:
- [ ] engine possession/control lease
- [ ] THAUMWORLD actor claim
- [ ] keep current actor claim implementation as the first concrete adapter
- [ ] isolate claim-specific naming from the more general session/lease layer where practical
- [ ] avoid renaming broad codepaths unless needed for a clean seam

Primary files:

- `src/shared/session_control.ts`
- relevant interface claim endpoints

Success criteria:

- [ ] actor claim remains working
- [ ] the project has a clear path to reusable possession semantics later
- [ ] Wi-Fi connection work does not depend on a full authority rewrite

### Phase 6: Event Bridge As Transport Bus

Status: not started

Intent:

Treat the event bridge as reusable transport infrastructure rather than a local-only helper.

Deliverables:

- [ ] document stable bridge message envelope fields:
- [ ] `type`
- [ ] `data`
- [ ] `delivery`
- [ ] document which behavior is engine-grade:
- [ ] session-authenticated subscribe
- [ ] connection-targeted delivery
- [ ] heartbeats
- [ ] invalidation notices
- [ ] preserve current ports and behavior during cleanup
- [ ] prepare the bridge for configurable host addresses

Primary files:

- `src/event_bridge/main.ts`
- `src/shared/event_bridge_client.ts`
- `src/mono_ui/websocket_client.ts`

Success criteria:

- [ ] bridge usage no longer assumes same-machine-only
- [ ] targeted delivery semantics remain intact
- [ ] transport docs match actual code behavior

### Phase 7: Engine Join Screen

Status: not started

Intent:

Replace app-specific join panels with one engine-owned join screen that both THAUMWORLD and ASCII painter can invoke from their boot flows.

Deliverables:

- [ ] create an engine join screen state/controller layer
- [ ] create a shared join screen module sized similarly to the main boot screen
- [ ] render a server/connection list styled more like the group UI than the current flat join list
- [ ] show local and saved manual connections in one list
- [ ] add actions under the list:
- [ ] `Join`
- [ ] `Refresh`
- [ ] `Add Connection`
- [ ] `Rename`
...
- [ ] `Forget`
- [ ] `Back`
- [ ] support keyboard and pointer selection cleanly
- [ ] support a generic `EngineJoinSelection` output that programs adapt after join succeeds

Primary files:

- `src/engine_launch/join_menu_types.ts`
- `src/engine_launch/join_controller.ts`
- `src/mono_ui/modules/join_directory_module.ts`

Success criteria:

- [ ] pressing `Join` opens a proper engine-owned join screen
- [ ] local host appears automatically when discoverable
- [ ] saved manual connections persist and remain visible even when offline
- [ ] the same join screen can be used by both game and painter boot flows

### Phase 8: Launcher Topology Cleanup

Status: not started

Intent:

Support remote-host client mode without assuming every client boot should spawn or probe a local host first.

Deliverables:

- [ ] make launcher/runtime docs distinguish:
- [ ] embedded local host+client
- [ ] local client attaching to local host
- [ ] remote client attaching to LAN/Wi-Fi host
- [ ] dedicated host later
- [ ] make host probing configurable by target address
- [ ] keep current `smart`, `host`, and `client` modes working locally
- [ ] avoid coupling client mode to localhost must be the host

Primary files:

- `scripts/dev_with_logs.ts`
- `scripts/launch_with_logs.js`
- `scripts/launcher_common.mjs`

Success criteria:

- [ ] a remote-client mode is architecturally valid even before a polished UI exists
- [ ] local smart boot still behaves as today

### Phase 9: Program Boot Integration

Status: not started

Intent:

Integrate the engine join screen into both programs while keeping their post-join behavior program-specific.

Deliverables:

- [ ] THAUMWORLD `Join World` opens the engine join screen instead of the current app-specific join panel
- [ ] THAUMWORLD keeps actor claim / character creation after join succeeds
- [ ] ASCII painter `Join` opens the engine join screen from its launch menu
- [ ] ASCII painter keeps authoritative document bootstrap after join succeeds
- [ ] remove assumptions that game and painter need identical top-level boot shells
...
- `src/canvas_app/painter_launch_adapter.ts`
- `src/canvas_app/painter_app_state.ts`

Success criteria:

- [ ] THAUMWORLD still reaches character claim/creation only after world join succeeds
- [ ] painter still reaches document bootstrap only after join succeeds
- [ ] both programs share the engine join-directory flow without losing program-specific post-join UX

### Phase 10: Manual Wi-Fi Join

Status: not started

Intent:

Reach the first non-local multiplayer milestone with the smallest surface area.

Deliverables:

- [ ] support connecting to a manually supplied host address
- [ ] bootstrap session against that host
- [ ] connect to event bridge WS on that host
- [ ] preserve reconnect token flow
- [ ] preserve lease refresh flow

Guardrails:

- [ ] no LAN auto-discovery yet
- [ ] no NAT or internet-hosting scope yet
- [ ] no renderer ownership expansion

Success criteria:

- [ ] a client can connect to `http://<lan-ip>:8787`
- [ ] a client can subscribe to `ws://<lan-ip>:8789`
- [ ] session resume still works
- [ ] claim or control lease refresh still works
- [ ] localhost mode still works unchanged

### TAI Validation Track

- [ ] add TAI-friendly join-directory selection hooks once the engine join screen exists
- [ ] add a TAI smoke for THAUMWORLD join -> actor claim path
- [ ] add a TAI smoke for painter join -> authoritative document bootstrap path
- [ ] ensure connection rows and actions can be targeted deterministically by id in tests
- [ ] log selected connection id, resolved transport, and probe result during join attempts for reproducible debugging

### Recommended Order

1. [ ] extract transport config
2. [ ] define the engine connection core
3. [ ] build engine probe + join directory helpers
4. [ ] clean up session service boundaries
5. [ ] clean up possession or control lease boundaries
6. [ ] formalize event bridge as transport bus
7. [ ] build the engine join screen
8. [ ] clean up launcher topology assumptions
9. [ ] integrate THAUMWORLD and painter boot flows
10. [ ] add manual Wi-Fi join path

### Definition Of Wi-Fi-Ready Scaffolding

This addendum is successful when all of the following are true:

- [ ] no multiplayer client path is hardcoded to localhost
- [ ] one transport config contract exists
- [ ] one engine connection contract exists
- [ ] one engine join-directory contract exists
- [ ] one session bootstrap contract exists
- [ ] current local multiplayer still works
- [ ] a client can connect to a host on LAN/Wi-Fi by manual address
- [ ] reconnect and claim leases survive that move without renderer changes
- [ ] THAUMWORLD and painter both use the same engine join screen before their program-specific post-join flow

### Risks

1. doing too much under the name of multiplayer
2. accidentally dragging renderer contracts into transport work
3. renaming authority concepts too broadly before the seams are stable
4. breaking current localhost flow while trying to support remote hosts
5. coupling discovery, boot, session, and claim changes into one risky pass

Mitigations:

- [ ] preserve localhost defaults first
- [ ] separate config extraction from behavior changes
- [ ] keep actor claim working as-is while introducing cleaner boundaries
- [ ] add manual host entry before any automatic discovery
- [ ] validate each seam with local mode before remote mode

### First Implementation Slice For This Direction

1. [ ] create `src/engine_multiplayer/connection_types.ts` with persistent connection and live probe types
2. [ ] create `src/engine_multiplayer/connection_store.ts` by migrating saved manual connection persistence out of app-specific code
3. [ ] make both game and painter consume the same engine connection-store API without changing their full boot UX yet
4. [ ] keep current local auto-detect behavior, but represent it as a synthetic engine connection entry
