...
- This plan does not change character movement controls (click-to-move vs realtime input), multi-level traversal, or pathing UX.
  Those decisions live in a separate plan: `docs/plans/2026_03_10_realtime_movement_plan.md`.
...
- [x] Create a Place "camera" controller with the same settings/behaviors as the ASCII painter camera.
...
- [x] Per-place tile instance grids exist on Place (`tiles_z0` and `tiles`) and are treated as defs+deltas.
  - Persistence stores only instance fields (not derived tags/display) and response-time augmentation is allowed for UI.
- [x] Finalize authoritative z=0 semantics (server-owned mutations + persistence rules)
  - Persistence rule: place files store only tile instance `kind` plus `tag_add/tag_remove` (and other non-derived instance fields like door/meta/contents).
  - Derived fields (`tags`, `display_*`, `container_glyphs`, `walkable/blocks_*`, `__derived_runtime`) are scrubbed on load and stripped on save.
  - Runtime rule semantics are tag-driven (OCCUPIES/COVER/DOOR/CONTAINER), resolved from tile defs + deltas.
...
- This phase defines collision semantics (`blocks_movement`) for simulation/pathing queries.
- Character movement *controls* and multi-level traversal are out of scope here (see realtime movement plan).
...
- [~] Render shaded cells for each world z
  ...
  - Deprioritized: incremental world draw into offscreen canvases (moved to `docs/plans/2026_03_07_advanced_rendering_plan.md`).
...
- [~] Extend existing shader tile payloads to carry tags/material context (no new resolver)
  - Implemented: `make_simple_tile_payload(..., tags?)` now passes tile tags through for shader modifiers.
    - `src/render_shaders/payload_builders.ts`
    - `src/mono_ui/modules/place_module.ts`
...
- [x] Add shared 3D math helpers and use them in perception + debug
- [x] LOS raycast uses `blocks_los`
- [x] Hearing/broadcast use 3D distance + culling
- [x] Add 3D debug visualization (outer bounds/intersection outlines)
...
- [x] `3DIFICATION_TEST PASS wall-top ground item present` appears when loading `eden_crossroads_tavern`.
- [x] `3DIFICATION_TEST PASS /api/place/items includes elevated ground item` appears when the ground cache refreshes.
- [x] `3DIFICATION_TEST PASS /api/place/items supports multi-z piles` appears for `eden_crossroads_tavern`.
- [x] `3DIFICATION_TEST PASS ground item cache supports multi-z piles` appears for `eden_crossroads_tavern`.
- [x] With focus plane set to the top layer, the wall-top item renders on the upper DOM layer at tile (7,5).
