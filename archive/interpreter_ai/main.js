...
const interpreter_sessions = new Map();
let current_actor_id = null;
...
function get_session_history(session_key) {
    return interpreter_sessions.get(session_key) ?? [];
}
function append_session_turn(session_key, user_text, assistant_text) {
...
function format_error_list(errors) {
...
function preprocess_communication_text(text) {
...
function build_interpreter_prompt(params) {
    ...
    if (!params.is_refinement) {
        ...
        if (params.ui_intent_verb || params.ui_action_cost || params.ui_target_ref) {
            base.push("", "UI overrides (trust these over ambiguous language):", `- intent_verb: ${params.ui_intent_verb ?? "(none)"}`, `- action_cost: ${params.ui_action_cost ?? "(none)"}`, `- target_ref: ${params.ui_target_ref ?? "(none)"}`);
        }
...
function has_positional_args(line) {
...
function has_named_args(line) {
    const args = get_command_args(line);
    if (!args)
        return false;
    return args.includes("=");
}
function replace_outside_quotes(input, regex, replacement) {
...
function add_arg(line, arg, value) {
...
function default_tool_for_verb(verb, actor_ref) {
    if (!verb)
        return null;
    if (verb === "COMMUNICATE")
        return `${actor_ref}.voice`;
    return `${actor_ref}.hands`;
}
function normalize_command_line(line, actor_ref, apply_defaults) {
    const issues = [];
    if (!is_command_line(line))
        return { line, issues: ["invalid_line"], changed: false };
    let next = line;
    let changed = false;
    ...
    if (normalized.changed)
        changed = true;
    if (has_positional_args(next))
        issues.push("positional_args");
...
function normalize_machine_text(text, actor_ref, apply_defaults) {
    const lines = text
        .split("\n")
        .map((line) => line.trim())
...
function is_valid_machine_text(text) {
    if (!text.trim())
        return false;
    const lines = text
        .split("\n")
...
function sanitize_machine_text(text) {
...
function detect_intent(text) {
    ...
    if (tokens.some((t) => system_tokens.has(t)))
        return { intent: "system", verb: null };
    const verb = detect_action_verb(text);
    if (verb)
        return { intent: "action", verb };
    if (is_question_like(text))
        return { intent: "unknown", verb: null };
    return { intent: "unknown", verb: null };
}
function normalize_entity_id(raw) {
    const trimmed = raw.trim();
    let id = trimmed;
    if (id.startsWith("actor."))
        id = id.slice("actor.".length);
    if (id.startsWith("npc."))
        id = id.slice("npc.".length);
    if (id.endsWith(".jsonc"))
        id = id.slice(0, -".jsonc".length);
    return { id, changed: id !== trimmed };
}
function get_active_actor_id() {
    if (current_actor_id)
...
function get_active_actor_ref() {
    const actor_id = get_active_actor_id();
    return `actor.${actor_id}`;
}
function resolve_communication_targets(text) {
...
function resolve_place_from_text(text) {
    const lowered = text.toLowerCase();
...
function get_actor_location() {
    ...
    const location = loaded.actor.location ?? {};
...
function build_place_tile_ref(place_id, tile) {
    const parts = place_id.split("_");
    if (parts.length < 2)
        return `place_tile.${place_id}.${tile.x}.${tile.y}`;
    const place_suffix = parts.pop();
    const region_id = parts.join("_");
    return `place_tile.${region_id}.${place_suffix}.${tile.x}.${tile.y}`;
}
function get_npcs_in_place(place_id) {
    if (!place_id) {
    ...
    for (const npc_hit of all_npcs) {
        ...
        if (!npc_result.ok)
            continue;
        const npc_location = npc_result.npc.location ?? {};
        const npc_place_id = npc_location.place_id;
...
function build_adjacent_tile_ref(location) {
    const dirs = [
        { x: 1, y: 0 },
        { x: -1, y: 0 },
        { x: 0, y: 1 },
...
function resolve_nearby_npc_target() {
    const location = get_actor_location();
    const npcs = get_npcs_in_place(location.place_id);
    if (npcs.length === 0) {
...
function resolve_communication_senses(text) {
    const lowered = text.toLowerCase();
    if (lowered.includes("smoke") || lowered.includes("signal"))
        return ["light"];
    return ["pressure"];
}
function infer_sense_context(text, senses) {
    const lowered = text.toLowerCase();
    const context = {};
...
function format_sense_context(context) {
    if (!context)
        return "";
...
function validate_and_fix_machine_text(text) {
    ...
    fixed = fixed.replace(/targets=\[\{ref[:=]\s*([^}]+)\}\]/g, 'targets=[$1]');
...
function build_communicate_command(text) {
    const actor_ref = get_active_actor_ref();
...
function build_communicate_template(text) {
    const actor_ref = get_active_actor_ref();
    const senses = resolve_communication_senses(text);
...
function build_action_template(verb, text) {
    const actor_ref = get_active_actor_ref();
    if (verb === "COMMUNICATE")
        return build_communicate_template(text);
    if (verb === "INSPECT")
    ...
    if (verb === "GUARD")
        return `${actor_ref}.GUARD(target=region_tile.0.0.0.0, tool=${actor_ref}.hands, action_cost=FULL)`;
    if (verb === "WORK")
        return `${actor_ref}.WORK(target=region_tile.0.0.0.0, tool=${actor_ref}.hands, action_cost=FULL)`;
    if (verb === "HOLD")
    ...
    if (verb === "HELP")
        return `${actor_ref}.HELP(target=npc.<id>, tool=${actor_ref}.hands, action_cost=FULL)`;
    if (verb === "GRAPPLE")
        return `${actor_ref}.GRAPPLE(target=npc.<id>, tool=${actor_ref}.hands, action_cost=FULL)`;
    if (verb === "ATTACK")
        return `${actor_ref}.ATTACK(target=npc.<id>, tool=${actor_ref}.hands, action_cost=FULL)`;
    if (verb === "USE")
        return `${actor_ref}.USE(target=${actor_ref}, tool=${actor_ref}.hands, action_cost=PARTIAL)`;
    if (verb === "CRAFT")
        return `${actor_ref}.CRAFT(tool=${actor_ref}.hands, components=[], result=item.result, action_cost=EXTENDED)`;
    return `${actor_ref}.INSPECT(target=region_tile.0.0.0.0, tool=${actor_ref}.hands, contexts=[region_tile.0.0.0.0])`;
}
function extract_explicit_ref(text) {
    const npc = text.match(/npc\.[a-zA-Z0-9_]+/);
    if (npc && npc[0] !== "npc.default_npc")
        return npc[0];
...
function resolve_npc_target_from_text(text) {
    const location = get_actor_location();
    const npcs = get_npcs_in_place(location.place_id);
    const lowered = text.toLowerCase();
    ...
    for (const npc of npcs) {
        if (npc.name && lowered.includes(npc.name.toLowerCase()))
            return `npc.${npc.id}`;
        if (npc.id && lowered.includes(npc.id.toLowerCase()))
            return `npc.${npc.id}`;
    }
    ...
    for (const word of words) {
        for (const npc of npcs) {
            if (npc.name && isSimilarName(word, npc.name))
                return `npc.${npc.id}`;
            if (npc.id && isSimilarName(word, npc.id))
                return `npc.${npc.id}`;
        }
    }
    return null;
}
function build_action_fallback(verb, text) {
    const actor_ref = get_active_actor_ref();
...
async function run_interpreter_ai(params) {
    ...
    try {
        ...
        log_ai_io_terminal('interpreter', params.text, stored_output, response.duration_ms, session_key, fullPrompt, response.content);
...
function message_contains_text(msg, needle) {
    if (!needle)
        return false;
    const text = msg.content ?? "";
    return text.toLowerCase().includes(needle.toLowerCase());
}
function parse_error_iteration(meta) {
    ...
    const error_stage = typeof meta?.error_stage === "string" ? meta?.error_stage : "";
    if (!error_stage.startsWith("interpretation_error_"))
        return undefined;
...
function post_tweak(text, original) {
    const input = {
    ...
    if (original.correlation_id !== undefined)
        input.correlation_id = original.correlation_id;
    return create_message(input);
}
// Note: update_outbox_message is now imported from outbox_store.ts for consistency
async function process_message(outbox_path, inbox_path, log_path, msg) {
    ...
    if (typeof msg.sender === "string" && msg.sender.length > 0) {
        ...
        if (loaded.ok)
            current_actor_id = normalized.id;
    }
    const meta = msg.meta ?? undefined;
    ...
    const original_text = typeof meta?.original_text === "string" ? meta?.original_text : msg.content ?? "";
    ...
    const error_list = Array.isArray(meta?.errors) ? meta?.errors : [];
    // UI overrides (optional)
    const override_verb_raw = meta?.intent_verb;
    ...
    const detected = (override_verb && ACTION_VERBS.includes(override_verb))
        ? { intent: "action", verb: override_verb }
        : detected_base;
    ...
    const requires_target = new Set(["COMMUNICATE", "ATTACK", "GRAPPLE", "HELP"]);
    const has_ui_target = typeof meta?.target_ref === "string" && String(meta?.target_ref).length > 0;
    ...
    if (detected.intent !== "action" || !detected.verb) {
        if (is_question_like(original_text)) {
            ...
            if (msg.correlation_id !== undefined)
                hint_msg.correlation_id = msg.correlation_id;
    ...
    if (detected.intent === "action" && detected.verb && ui_target_ref) {
        ...
        if (local_text) {
            ...
            if (processing.message.correlation_id !== undefined)
                response_msg.correlation_id = processing.message.correlation_id;
            if (processing.message.conversation_id !== undefined)
                response_msg.conversation_id = processing.message.conversation_id;
    ...
    const response_text = await run_interpreter_ai({
        ...
        errors: error_list,
        previous_machine_text: typeof meta?.machine_text === "string" ? meta?.machine_text : undefined,
        intent: detected.intent,
        action_verb: detected.verb,
        action_template,
        ui_intent_verb: override_verb || null,
        ui_action_cost: typeof meta?.action_cost === "string" ? meta.action_cost : null,
        ui_target_ref: typeof meta?.target_ref === "string" ? meta.target_ref : null,
    });
    ...
    if (detected.verb === "COMMUNICATE") {
        const meta_session_id = msg.meta?.session_id;
        const session_id = typeof meta_session_id === "string" && meta_session_id.length > 0
            ? meta_session_id
            : getSessionMeta()?.session_id ?? "session";
...
async function tick(outbox_path, inbox_path, log_path) {
    ...
    const candidates = outbox.messages.filter((m) => m.stage === "interpreter_ai" && m.status === "sent" && isCurrentSession(m));
