...
import { get_default_world_path, get_world_path } from "../engine/paths.js";
...
export function ensure_world_exists(slot: number): WorldLookupResult {
    ...
    const world_dir = world_path.replace(/\\world\.jsonc$/i, "");
    ...
    if (!fs.existsSync(template_path)) {
        const todo = `Default world template missing. TODO: create ${template_path}`;
...
export function get_world_tile(slot: number, x: number, y: number): WorldLookupResult {
    ...
    if (!tile) {
        const todo = `World tile not found at ${key}. TODO: create world tile in ${world.path}`;
...
export function get_region_tile(slot: number, world_x: number, world_y: number, region_x: number, region_y: number): RegionLookupResult {
    ...
    if (!region) {
        const todo = `Region tile not found at (${region_x},${region_y}) in world tile ${key}. TODO: create region tile.`;
