...
export function build_physics_tag_flags(tags: any): PhysicsTagFlags {
...
export function resolve_tile_physics_tags(tile: any): PhysicsTagFlags {
  ...
  return build_physics_tag_flags(resolved?.effective_tags ?? (Array.isArray(tile?.tags) ? tile.tags : []));
}
...
export function resolve_structure_physics_tags(structure: any): PhysicsTagFlags {
  ...
  const resolved = def_id
    ...
    : null;
  return build_physics_tag_flags(resolved?.effective_tags ?? []);
}
...
export function resolve_inline_item_physics_tags(item_any: any): PhysicsTagFlags {
  ...
  return build_physics_tag_flags(resolved?.effective_tags ?? (Array.isArray(item_any?.tags) ? item_any.tags : []));
}
