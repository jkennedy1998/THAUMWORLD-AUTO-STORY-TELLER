...
async function migrate_npc(slot, npc_id) {
    ...
    const npc_name = npc.name || npc_id;
    ...
    if (location.place_id) {
        return {
            ...
            new_place_id: location.place_id,
    ...
    if (!place_id) {
        ...
        if (world_tile && region_tile) {
            ...
            if (region_result.ok) {
                const region_id = region_result.region.id || "";
...
function format_old_location(location) {
    const world = location.world_tile;
    const region = location.region_tile;
...
//# sourceMappingURL=migrate_npcs_to_places.js.map